...
const makeEventBuffer = (logger) => {
    ...
    const BUFFER_TIMEOUT_MS = 15000; // 15 seconds safety valve (was 30s — halved for faster recovery)
        ...
        createBufferedFunction(work) {
            return async (...args) => {
                ...
                try {
                    ...
                    // If this is the only buffer, schedule a fast flush
                    if (bufferCount === 1) {
                        setTimeout(() => {
                            if (!flushed && isBuffering && bufferCount === 1) {
                                flushed = true;
                                flush();
                            }
                        }, 50); // 50ms — faster than original 100ms
                    }
                    return result;
                }
                catch (error) {
